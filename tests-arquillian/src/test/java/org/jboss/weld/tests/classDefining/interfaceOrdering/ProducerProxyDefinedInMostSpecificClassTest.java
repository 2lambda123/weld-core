...
/**
 * Verifies that a proxy for a producer-created bean starts with the package and
 * class of the most concrete class/interface. This should prevent problems in
 * JPMS where each class/interface can come from various module that don't have
...
public class ProducerProxyDefinedInMostSpecificClassTest {
    ...
    return ShrinkWrap
        .create(BeanArchive.class,
                Utils.getDeploymentNameAsHash(
                    ProducerProxyDefinedInMostSpecificClassTest.class))
  ...
  public void testProxyNameForInterfaceBasedBean() {
    ...
    // in this case, we want the package and class name to the that of B which
    // is the most specific interface implemented
    Assert.assertEquals(B.class.getName(),
                        someB.getClass().getName().substring(
                            0, someB.getClass().getName().indexOf("$")));
  }
  ...
  public void testProxyNameForClassBasedBean() {
    ...
    two.pingTwo();
    // then assert its name up to the first occurrence of "$"
    // in this case, we want the package and class name to the that of Two which
    // is the most specific class of the bean
    Assert.assertEquals(Two.class.getName(),
                        two.getClass().getName().substring(
                            0, two.getClass().getName().indexOf("$")));
  }
}

...
public class BeanConfiguratorTest {
  ...
  public static WebArchive createTestArchive() {
    return ShrinkWrap
        .create(WebArchive.class,
                Utils.getDeploymentNameAsHash(BeanConfiguratorTest.class,
  ...
  public void testConfigurator(BeanManager beanManager) throws Exception {
    ...
    Foo foo1 = (Foo)beanManager.getReference(
        fooBean, Foo.class, beanManager.createCreationalContext(fooBean));
    Foo foo2 = (Foo)beanManager.getReference(
        fooBean, Foo.class, beanManager.createCreationalContext(fooBean));
    ...
    Foo foo = (Foo)beanManager.getReference(
        fooBean, Foo.class, beanManager.createCreationalContext(fooBean));
    foo.ping();

    // bean is deliberately created via new creational context and
    // BM.getReference
    ...
    CreationalContext<Integer> ctx =
        beanManager.createCreationalContext(randomBean);
    Integer random =
        (Integer)beanManager.getReference(randomBean, Integer.class, ctx);
    ...
    Instance<Integer> integerInstance = beanManager.createInstance().select(
        Integer.class, Random.Literal.INSTANCE);
    assertEquals(0, DependentBean.TIMES_DESTROY_INVOKED);
    ...
    DependentBean.resetCounter();
    BuilderExtension.DISPOSED.set(false);
    Instance<BeanInjectingSyntheticInteger> injectingBeanInstance =
        beanManager.createInstance().select(
            BeanInjectingSyntheticInteger.class);
    ...
    Long anotherRandom = (Long)beanManager.getReference(
        anotherRandomBean, Long.class,
    ...
    beans = beanManager.getBeans(new TypeLiteral<List<String>>() {}.getType(),
    ...
    List<String> list = (List<String>)beanManager.getReference(
        listBean, new TypeLiteral<List<String>>() {}.getType(),
    ...
    fooBean = (Bean<Foo>)beans.iterator().next();
    assertEquals(Dependent.class, fooBean.getScope());
    Foo randomFoo = (Foo)beanManager.getReference(
    ...
    assertEquals(1, beans.size());
    Bean<Configuration> configBean =
        (Bean<Configuration>)beans.iterator().next();
    assertEquals(Dependent.class, configBean.getScope());
    Configuration configuration = (Configuration)beanManager.getReference(

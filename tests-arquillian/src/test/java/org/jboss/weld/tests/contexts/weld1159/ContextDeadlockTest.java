...
/**
 *
 * Reproduces deadlock between creation locks in the application and session
 * context.
 *
 ...
 *
 * We use a {@link CountDownLatch} to simulate that both threads get into state
 * when the first bean of the pair (SessionScopedFoo and ApplicationScopedFoo)
 * is created and before the dependency is created. This causes deadlock because
 * the first thread
 *
...
public class ContextDeadlockTest {
  ...
  @ArquillianResource private volatile URL url;
  ...
  public static Archive<?> getDeployment() {
    return ShrinkWrap
        .create(WebArchive.class,
                Utils.getDeploymentNameAsHash(ContextDeadlockTest.class,
                                              Utils.ARCHIVE_TYPE.WAR))
        .addClasses(AbstractBean.class, ApplicationScopedFoo.class,
                    ApplicationScopedBar.class, SessionScopedFoo.class,
                    SessionScopedBar.class, TestServlet.class)
  ...
  private class ConcurrentRequest implements Callable<Void> {
    ...
    public ConcurrentRequest(String url) { this.url = url; }
    ...
    public Void call() throws Exception {
      try {
        ...
        throw new RuntimeException(
            "The request for " + url +
            " timed out. It is very likely that a deadlock occured.");
      }
      return null;
    }
  }

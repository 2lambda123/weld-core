...
public class RequestScopedCacheLeakTest {
  ...
  public static WebArchive getDeployment() {
    return ShrinkWrap
        .create(WebArchive.class,
                Utils.getDeploymentNameAsHash(RequestScopedCacheLeakTest.class,
  ...
  public void test() throws Exception {
    ...
    for (int i = 0; i < 100; i++) {
      // now send out normal requests to see if they are affected by the
      // thread's broken state
      String result = sendRequest(webClient, i, false);
      Assert.assertFalse("Invalid state detected after " + (i + 1) +
                             " requests",
                         result.startsWith("bar"));
    }
  }
  ...
  private String getPath(String test, int sequence, boolean poison) {
    String path =
        contextPath + "/servlet?action=" + test + "&sequence=" + sequence;
    if (poison) {
      path += "&poison=true";
    }
    return path;

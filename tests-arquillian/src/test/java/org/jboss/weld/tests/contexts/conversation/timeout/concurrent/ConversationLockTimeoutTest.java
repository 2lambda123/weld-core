...
public class ConversationLockTimeoutTest {
  ...
  public void testLongerConversationLockTimeout() throws Exception {
    ...
    TextPage initPage =
        client.getPage(url + "inspect?mode=" + InspectServlet.MODE_INIT);
    String cid = extractCid(initPage.getContent());
    Assert.assertNotNull(cid);
    Assert.assertFalse(cid.isEmpty());
    String jsessionid =
        client.getCookieManager().getCookie(JSESSIONID).getValue();
    Assert.assertNotNull(jsessionid);
    Assert.assertFalse(jsessionid.isEmpty());

    ExecutorService executorService = Executors.newFixedThreadPool(2);
    WebRequest longTask =
        new WebRequest(InspectServlet.MODE_LONG_TASK, url, cid, jsessionid);
    WebRequest busyRequest =
        new WebRequest(InspectServlet.MODE_BUSY_REQUEST, url, cid, jsessionid);

    final Future<String> longTaskFuture = executorService.submit(longTask);
    Timer timer = Timer.startNew(1000l);
    final Future<String> busyRequestFuture =
        executorService.submit(busyRequest);
    timer.setSleepInterval(100l)
        .setDelay(2, TimeUnit.SECONDS)
        .addStopCondition(
            () -> longTaskFuture.isDone() || busyRequestFuture.isDone())
        .start();
  ...
  private class WebRequest implements Callable<String> {
    ...
    public WebRequest(String mode, URL contextPath, String cid,
                      String jsessionid) {
      super();
      this.mode = mode;
    ...
    public String call() throws Exception {
      ...
      client.getCookieManager().addCookie(
          new Cookie(contextPath.getHost(), JSESSIONID, jsessionid));

      Page page =
          client.getPage(contextPath + "inspect?mode=" + mode + "&cid=" + cid);

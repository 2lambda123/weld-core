...
public class CreateAnnotatedTypeWithIdTest {
  ...
  public static Archive<?> createTestArchive() {
    return ShrinkWrap
        .create(BeanArchive.class, Utils.getDeploymentNameAsHash(
                                       CreateAnnotatedTypeWithIdTest.class))
  ...
  public void testCreateAnnotatedTypeWithId(BeanManagerImpl beanManager) {
    AnnotatedType<Component> annotatedType =
        beanManager.createAnnotatedType(Component.class);
    ...
    // we need to define this class in a new CL to avoid duplicate declaration -
    // hence we use CFW way to define it
    ClassFile componentClassFile =
        new ClassFile(Component.class.getName(), Object.class.getName(),
                      new SimpleClassLoader(Component.class.getClassLoader()),
                      factory, new String[] {});
    // Add void pong()
    CodeAttribute b =
        componentClassFile
            .addMethod(AccessFlag.of(AccessFlag.PUBLIC, AccessFlag.SYNTHETIC),
                       "pong", "V")
            .getCodeAttribute();
    b.returnInstruction();
    Class<?> componentClass = componentClassFile.define();
    @SuppressWarnings("unchecked")
    BackedAnnotatedType<Component> newAnnotatedType =
        (BackedAnnotatedType<Component>)beanManager.createAnnotatedType(
            componentClass, componentClass.getName() +
                                componentClass.getClassLoader().hashCode());
    ...
    beanManager.disposeAnnotatedType(
        componentClass,
        componentClass.getName() + componentClass.getClassLoader().hashCode());
    assertNull(beanManager.getServices()
  ...
  private static class SimpleClassLoader extends ClassLoader {
    ...
    SimpleClassLoader(ClassLoader parent) { super(parent); }

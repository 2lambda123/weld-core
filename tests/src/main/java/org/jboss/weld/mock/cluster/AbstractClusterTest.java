...
public class AbstractClusterTest {
  ...
  public void beforeClass() throws Exception {
    ...
    getInstanceField().set(
        null, new SwitchableSingletonProvider().create(Container.class));
  }
  ...
  protected TestContainer bootstrapContainer(int id,
                                             Collection<Class<?>> classes) {
    // Bootstrap container
    SwitchableSingletonProvider.use(id);

    TestContainer container = new TestContainer(
        new FlatDeployment(new BeanDeploymentArchiveImpl(classes)));
    container.getDeployment().getServices().add(ProxyServices.class,
                                                new WeldDefaultProxyServices());
  ...
  protected static BeanManagerImpl getBeanManager(TestContainer container) {
    return (BeanManagerImpl)container.getBeanManager(
  ...
  protected void use(int id) { SwitchableSingletonProvider.use(id); }

  protected void replicateSession(int fromId, TestContainer fromContainer,
                                  int toId, TestContainer toContainer)
      throws Exception {
    ...
    BoundSessionContext sessionContext =
        toContainer.instance().select(BoundSessionContext.class).get();
    sessionContext.deactivate();
    sessionContext.dissociate(toContainer.getSessionStore());

    // then copy them into the other session store
    toContainer.getSessionStore().putAll(
        Reflections.<Map<String, Object>>cast(deserialize(bytes)));

...
/**
 * Takes care of setting up and tearing down CDI contexts around an HTTP request
 * and dispatching context lifecycle events.
 *
...
public class HttpContextLifecycle implements Service {
  ...
  public static final String ASYNC_STARTED_ATTR_NAME =
      "org.jboss.weld.context.asyncStarted";

  private static final String HTTP_SESSION =
      "org.jboss.weld." + HttpSession.class.getName();

  private static final String INCLUDE_HEADER =
      "javax.servlet.include.request_uri";
  private static final String FORWARD_HEADER =
      "javax.servlet.forward.request_uri";
  private static final String REQUEST_DESTROYED =
      HttpContextLifecycle.class.getName() + ".request.destroyed";

  private static final String GUARD_PARAMETER_NAME =
      "org.jboss.weld.context.ignore.guard.marker";
  ...
  private static final ThreadLocal<Counter> nestedInvocationGuard =
      new ThreadLocal<HttpContextLifecycle.Counter>();
  ...
  public HttpContextLifecycle(
      ...
      boolean ignoreForwards, boolean ignoreIncludes,
      boolean lazyConversationContext, boolean nestedInvocationGuardEnabled) {
    this.beanManager = beanManager;
    this.conversationContextActivator =
        new ConversationContextActivator(beanManager, lazyConversationContext);
    ...
    this.requestInitializedEvent = FastEvent.of(
        HttpServletRequest.class, beanManager, Initialized.Literal.REQUEST);
    this.requestBeforeDestroyedEvent = FastEvent.of(
        HttpServletRequest.class, beanManager, BeforeDestroyed.Literal.REQUEST);
    this.requestDestroyedEvent = FastEvent.of(
        HttpServletRequest.class, beanManager, Destroyed.Literal.REQUEST);
    this.sessionInitializedEvent = FastEvent.of(HttpSession.class, beanManager,
                                                Initialized.Literal.SESSION);
    this.sessionBeforeDestroyedEvent = FastEvent.of(
        HttpSession.class, beanManager, BeforeDestroyed.Literal.SESSION);
    this.sessionDestroyedEvent =
        FastEvent.of(HttpSession.class, beanManager, Destroyed.Literal.SESSION);
    this.servletApi =
        beanManager.getServices().get(ServletApiAbstraction.class);
    this.servletContextService =
        beanManager.getServices().get(ServletContextService.class);
    this.nestedInvocationGuardEnabled = nestedInvocationGuardEnabled;
    this.container = Container.instance(beanManager);
    BeanDeploymentModules beanDeploymentModules =
        beanManager.getServices().get(BeanDeploymentModules.class);
    this.module = beanDeploymentModules != null
                      ? beanDeploymentModules.getModule(beanManager)
                      : null;
  }

  private HttpSessionDestructionContext getSessionDestructionContext() {
    if (sessionDestructionContextCache == null) {
      this.sessionDestructionContextCache =
          beanManager.instance()
              .select(HttpSessionDestructionContext.class)
              .get();
    }
  ...
  private HttpSessionContext getSessionContext() {
    if (sessionContextCache == null) {
      this.sessionContextCache =
          beanManager.instance().select(HttpSessionContext.class).get();
    }
  ...
  public HttpRequestContext getRequestContext() {
    if (requestContextCache == null) {
      this.requestContextCache =
          beanManager.instance().select(HttpRequestContext.class).get();
    }
  ...
  public void contextDestroyed(ServletContext ctx) {
    // TODO WELD-2282 Firing these two right after each other does not really
    // make sense
  ...
  private void fireEventForApplicationScope(ServletContext ctx,
      ...
      synchronized (container) {
        if (module.isWebModule()) {
          ...
          final EventMetadata metadata = new EventMetadataImpl(
              ServletContext.class, null, Collections.singleton(qualifier));
          beanManager.getAccessibleLenientObserverNotifier().fireEvent(
              ServletContext.class, ctx, metadata, qualifier);
        }
      }
    }
  }
  ...
  public void sessionDestroyed(HttpSession session) {
    ...
    if (destroyed) {
      // we are outside of a request (the session timed out) and therefore the
      // session was destroyed immediately we can fire the
      // @Destroyed(SessionScoped.class) event immediately
      ...
      if (getRequestContext() instanceof HttpRequestContextImpl) {
        HttpServletRequest request =
            Reflections.<HttpRequestContextImpl>cast(getRequestContext())
  ...
  public void requestInitialized(HttpServletRequest request,
    ...
    if (nestedInvocationGuardEnabled) {
      ...
      if (counter != null && marker != null) {
        // this is a nested invocation, increment the counter and ignore this
        // invocation
        counter.value++;
        return;
      } else {
        if (counter != null && marker == null) {
          /*
           * This request has not been processed yet but the guard is set
           * already. That indicates, that the guard leaked from a previous
           * request processing - most likely the Servlet container did not
           * invoke listener methods symmetrically. Log a warning and recover by
    ...
    try {
      ...
      /*
       * If the servlet container happens to call the destroyed callback again,
       * ignore it.
       */
  ...
  public void requestDestroyed(HttpServletRequest request) {
    ...
    try {
      ...
      /*
       * If this request has been switched to async then do not invalidate the
       * context now as it will be invalidated at the end of the async
       * operation.
       */
      if (servletApi.isAsyncSupported() && servletApi.isAsyncStarted(request)) {
        // Note that we can't use isAsyncStarted() because it may return false
        // after dispatch
      ...
      // Catch block is inside the activator method so that we're able to log
      // the context
  ...
  public void
  setConversationActivationEnabled(boolean conversationActivationEnabled) {
  ...
  /**
   * Some Servlet containers fire HttpServletListeners for include requests
   * (inner requests caused by calling the include method of RequestDispatcher).
   * This causes problems with context shut down as context manipulation is not
   * reentrant. This method detects if this request is an included request or
   * not.
   */
  ...
  /**
   * Some Servlet containers fire HttpServletListeners for forward requests
   * (inner requests caused by calling the forward method of RequestDispatcher).
   * This causes problems with context shut down as context manipulation is not
   * reentrant. This method detects if this request is an forwarded request or
   * not.
   */
  ...
  /**
   * The way servlet containers react to an exception that occurs in a {@link
   * ServletRequestListener} differs among servlet listeners. In certain
   * containers the destroyed callback may be invoked multiple times, causing
   * the latter invocations to fail as thread locals have already been unset. We
   * use the
   * {@link #REQUEST_DESTROYED} flag to indicate that all further invocations of
   * the
   * {@link
   * ServletRequestListener#requestDestroyed(javax.servlet.ServletRequestEvent)}
   * should be ignored by Weld.
   */
  ...
  private void safelyDeactivate(ManagedContext context,
                                HttpServletRequest request) {
    try {
      context.deactivate();
    } catch (Exception e) {

...
public class InterceptedSubclassFactory<T> extends ProxyFactory<T> {
  ...
  static final String
      COMBINED_INTERCEPTOR_AND_DECORATOR_STACK_METHOD_HANDLER_CLASS_NAME =
          CombinedInterceptorAndDecoratorStackMethodHandler.class.getName();
  static final String[] INVOKE_METHOD_PARAMETERS = new String[] {
      makeDescriptor(Stack.class), LJAVA_LANG_OBJECT, LJAVA_LANG_REFLECT_METHOD,
  ...
  public InterceptedSubclassFactory(
      String contextId, Class<?> proxiedBeanType,
   ...
   * @param enhancedMethodSignatures a restricted set of methods that need to be
   *     intercepted
   */
  public InterceptedSubclassFactory(
      String contextId, Class<?> proxiedBeanType,
      Set<? extends Type> typeClosure, String proxyName, Bean<?> bean,
      Set<MethodSignature> enhancedMethodSignatures,
      Set<MethodSignature> interceptedMethodSignatures) {
  ...
  public void addInterfacesFromTypeClosure(Set<? extends Type> typeClosure,
    ...
    // these interfaces we want to scan for method and our proxies will
    // implement them
    for (Class<?> c : proxiedBeanType.getInterfaces()) {
      addInterface(c);
    }
    // now we need to go deeper in hierarchy and scan those interfaces for
    // additional interfaces with default impls
  ...
  private boolean skipIfBridgeMethod(Method method,
                                     Collection<Method> classDeclaredMethods) {
    if (method.isBridge()) {
      // if it's a bridge method, we need to see if the class also contains an
      // actual "impl" of that method if it does, we can skip this method, if it
      // doesn't we will need to intercept it
      for (Method declaredMethod : classDeclaredMethods) {
        ...
        if (method.getName().equals(declaredMethod.getName())) {
          ...
          if (!Modifier.isInterface(
                  declaredMethod.getDeclaringClass().getModifiers())) {
            if (method.getReturnType().getName().equals(
              ...
              // or the method we compare against is abstract meaning the bridge
              // overrides it both cases are a match
              return true;
            } else {
  ...
  protected void addMethodsFromClass(ClassFile proxyClassType,
    ...
    try {
      ...
      final Set<BridgeMethod> processedBridgeMethods =
          new HashSet<BridgeMethod>();

      // Add all methods from the class hierarchy
      Class<?> cls = getBeanType();
      while (cls != null) {
        Set<BridgeMethod> declaredBridgeMethods = new HashSet<BridgeMethod>();
        Collection<Method> classDeclaredMethods = Arrays.asList(
            AccessController.doPrivileged(new GetDeclaredMethodsAction(cls))
                .clone());
        for (Method method : classDeclaredMethods) {

          final MethodSignatureImpl methodSignature =
              new MethodSignatureImpl(method);

          if (!Modifier.isFinal(method.getModifiers()) &&
              !skipIfBridgeMethod(method, classDeclaredMethods) &&
              enhancedMethodSignatures.contains(methodSignature) &&
              !finalMethods.contains(methodSignature) &&
              CommonProxiedMethodFilters.NON_PRIVATE_WITHOUT_PACK_PRIVATE_PARAMS
                  .accept(method, getProxySuperclass()) &&
              !bridgeMethodsContainsMethod(
                  processedBridgeMethods, methodSignature,
            ...
            try {
              ...
              if (interceptedMethodSignatures.contains(methodSignature)) {
                ...
                // we still need to override and push
                // InterceptionDecorationContext stack to prevent full
                // interception
                ClassMethod classMethod = proxyClassType.addMethod(method);
                new RunWithinInterceptionDecorationContextGenerator(classMethod,
                  ...
                  void doWork(CodeAttribute b, ClassMethod classMethod) {
                    if (Modifier.isPrivate(classMethod.getAccessFlags())) {
                      // Weld cannot use invokespecial to invoke a private
                      // method from the superclass
                      invokePrivateMethodHandler(b, classMethod, methodInfo,
                                                 staticConstructor);
                    } else {
                      // build the bytecode that invokes the super class method
                      // directly
                      b.aload(0);
                      // create the method invocation
                      b.loadMethodParameters();
                      b.invokespecial(methodInfo.getDeclaringClass(),
                                      methodInfo.getName(),
            ...
            if (method.isBridge()) {
              BridgeMethod bridgeMethod = new BridgeMethod(
                  methodSignature, method.getGenericReturnType());
              if (!hasAbstractPackagePrivateSuperClassWithImplementation(
                      cls, bridgeMethod)) {
      ...
      // We want to iterate over pre-defined interfaces
      // (getAdditionalInterfaces()) and also over those we discovered earlier
      // (interfacesToInspect)
      ...
      for (Class<?> c : allInterfaces) {
        for (Method method : c.getMethods()) {
          ...
          // For interfaces we do not consider return types when going through
          // processed bridge methods
          if (enhancedMethodSignatures.contains(signature) &&
              !bridgeMethodsContainsMethod(
                  processedBridgeMethods, signature, null,
                  Modifier.isAbstract(method.getModifiers()))) {
            try {
              MethodInformation methodInfo =
                  new RuntimeMethodInformation(method);
              if (interceptedMethodSignatures.contains(signature) &&
                ...
                // we only want to add default methods, rest is abstract and
                // cannot be invoked
          ...
          if (method.isBridge()) {
            processedBridgeMethods.add(
                new BridgeMethod(signature, method.getGenericReturnType()));
          }
        }
      }
    } catch (Exception e) {
  ...
  /**
   * Returns true if super class of the parameter exists and is abstract and
   * package private. In such case we want to omit such method.
   *
   * See WELD-2507 and Oracle issue -
   * https://bugs.java.com/view_bug.do?bug_id=6342411
   *
   * @return true if the super class exists and is abstract and package private
   */
  private boolean hasAbstractPackagePrivateSuperClassWithImplementation(
      Class<?> clazz, BridgeMethod bridgeMethod) {
    Class<?> superClass = clazz.getSuperclass();
    while (superClass != null) {
      if (Modifier.isAbstract(superClass.getModifiers()) &&
          Reflections.isPackagePrivate(superClass.getModifiers())) {
        // if superclass is abstract, we need to dig deeper
        for (Method method : superClass.getDeclaredMethods()) {
          if (bridgeMethod.signature.matches(method) &&
              method.getGenericReturnType().equals(bridgeMethod.returnType) &&
              !Reflections.isAbstract(method)) {
            // this is the case we are after -> methods have same signature and
            // the one in super class has actual implementation
            return true;
          }
        }
      }
  ...
  private boolean
  bridgeMethodsContainsMethod(Set<BridgeMethod> processedBridgeMethods,
    ...
    for (BridgeMethod bridgeMethod : processedBridgeMethods) {
      if (bridgeMethod.signature.equals(signature)) {
        // method signature is equal (name and params) but return type can still
        // differ
        if (returnType != null) {
          if (bridgeMethod.returnType.equals(Object.class) ||
            ...
            if (bridgeMethod.returnType instanceof Class &&
                returnType instanceof TypeVariable) {
              // in this case we have encountered a bridge method with specific
              // return type in subclass and we are observing a TypeVariable
              // return type in superclass, this is a match
              return true;
            } else {
              // as a last resort, we simply check equality of return Type
  ...
  protected void createInterceptorBody(ClassMethod method,
                                       MethodInformation methodInfo,
                                       boolean delegateToSuper,
                                       ClassMethod staticConstructor) {

    invokeMethodHandler(method, methodInfo, true, DEFAULT_METHOD_RESOLVER,
                        delegateToSuper, staticConstructor);
  }

  private void createDelegateToSuper(ClassMethod classMethod,
                                     MethodInformation method) {
    createDelegateToSuper(classMethod, method,
   ...
   */
  protected void
  invokeMethodHandler(ClassMethod method, MethodInformation methodInfo,
                      boolean addReturnInstruction,
                      BytecodeMethodResolver bytecodeMethodResolver,
                      boolean addProceed, ClassMethod staticConstructor) {
    ...
    if (addProceed) {
      ...
      if (!Reflections.isDefault(methodInfo.getMethod()) &&
          !Modifier.isPrivate(method.getAccessFlags())) {
    ...
    bytecodeMethodResolver.getDeclaredMethod(
        method, methodInfo.getDeclaringClass(), methodInfo.getName(),
        methodInfo.getParameterTypes(), staticConstructor);

    if (addProceed) {
      if (Modifier.isPrivate(method.getAccessFlags())) {
        // If the original method is private we can't use
        // WeldSubclass.method$$super() as proceed
        bytecodeMethodResolver.getDeclaredMethod(
            method, methodInfo.getDeclaringClass(), methodInfo.getName(),
            methodInfo.getParameterTypes(), staticConstructor);
      } else {
        bytecodeMethodResolver.getDeclaredMethod(
            method, method.getClassFile().getName(),
    ...
    b.invokeinterface(StackAwareMethodHandler.class.getName(),
                      INVOKE_METHOD_NAME, LJAVA_LANG_OBJECT,
  ...
  protected void addSpecialMethods(ClassFile proxyClassType,
    ...
    try {
      ...
      generateGetTargetInstanceBody(
          proxyClassType.addMethod(getInstanceMethod));
      generateGetTargetClassBody(
          proxyClassType.addMethod(getInstanceClassMethod));
  ...
  private void createDelegateMethod(ClassFile proxyClassType, Method method,
                                    MethodInformation methodInformation) {
    int modifiers =
        (method.getModifiers() | AccessFlag.SYNTHETIC | AccessFlag.PRIVATE) &
        ~AccessFlag.PUBLIC & ~AccessFlag.PROTECTED;
    ClassMethod delegatingMethod = proxyClassType.addMethod(
        modifiers, method.getName() + SUPER_DELEGATE_SUFFIX,
        DescriptorUtils.makeDescriptor(method.getReturnType()),
        DescriptorUtils.parameterDescriptors(method.getParameterTypes()));
    delegatingMethod.addCheckedExceptions(
        (Class<? extends Exception>[])method.getExceptionTypes());
    createDelegateToSuper(delegatingMethod, methodInformation);
  }

  private void invokePrivateMethodHandler(CodeAttribute b,
                                          ClassMethod classMethod,
                                          MethodInformation methodInfo,
                                          ClassMethod staticConstructor) {
    try {
      classMethod.getClassFile().addField(AccessFlag.PRIVATE,
    ...
    b.aload(0);
    b.getfield(classMethod.getClassFile().getName(),
               PRIVATE_METHOD_HANDLER_FIELD_NAME,
               DescriptorUtils.makeDescriptor(MethodHandler.class));
    // 2. Load this
    b.aload(0);
    // 3. Load method
    DEFAULT_METHOD_RESOLVER.getDeclaredMethod(
        classMethod, methodInfo.getDeclaringClass(), methodInfo.getName(),
        methodInfo.getParameterTypes(), staticConstructor);
    // 4. No proceed method
    b.aconstNull();
  ...
  /**
   * If the given instance represents a proxy and its class is synthetic and its
   * class name ends with {@value #PROXY_SUFFIX}, attempt to find the
   * {@value #PRIVATE_METHOD_HANDLER_FIELD_NAME} field and set its value to
   * {@link PrivateMethodHandler#INSTANCE}.
   *
   * @param instance
   */
  public static <T> void setPrivateMethodHandler(T instance) {
    if (instance instanceof ProxyObject && instance.getClass().isSynthetic() &&
        ...
        SecurityActions.hasDeclaredField(instance.getClass(),
                                         PRIVATE_METHOD_HANDLER_FIELD_NAME)) {
      try {
        Field privateMethodHandlerField = SecurityActions.getDeclaredField(
            instance.getClass(), PRIVATE_METHOD_HANDLER_FIELD_NAME);
  ...
  private static class BridgeMethod {
    ...
    public int hashCode() {
      ...
      int result = 1;
      result =
          prime * result + ((returnType == null) ? 0 : returnType.hashCode());
      result =
    ...
    public String toString() {
      return new StringBuilder()
          ...
          .append(Arrays.toString(signature.getParameterTypes())
                      .replace('[', '(')
                      .replace(']', ')'))
          .toString();
    }

...
public class ProxyFactory<T> implements PrivilegedAction<T> {
  ...
  // choose different package from what we have in tests to distinguish it
  // clearly
  public static final String WELD_PROXY_PREFIX =
      "org.jboss.weld.generated.proxies";
  public static final String DEFAULT_PROXY_PACKAGE =
      WELD_PROXY_PREFIX + ".default";
  ...
  protected static final BytecodeMethodResolver DEFAULT_METHOD_RESOLVER =
      new DefaultBytecodeMethodResolver();
  ...
  protected static final String LBEAN_IDENTIFIER =
      "Lorg/jboss/weld/serialization/spi/BeanIdentifier;";
   ...
   */
  public ProxyFactory(String contextId, Class<?> proxiedBeanType,
                      Set<? extends Type> typeClosure, Bean<?> bean) {
  ...
  public ProxyFactory(String contextId, Class<?> proxiedBeanType,
    ...
    this(contextId, proxiedBeanType, typeClosure,
         getProxyName(contextId, proxiedBeanType, typeClosure, bean), bean,
         forceSuperClass);
  }
   ...
   */
  public ProxyFactory(String contextId, Class<?> proxiedBeanType,
                      Set<? extends Type> typeClosure, String proxyName,
                      Bean<?> bean) {
  ...
  public ProxyFactory(String contextId, Class<?> proxiedBeanType,
    ...
    this.configuration =
        Container.instance(contextId).deploymentManager().getServices().get(
            WeldConfiguration.class);
    ...
    if (forceSuperClass ||
        (superClass.equals(Object.class) && additionalInterfaces.isEmpty())) {
    ...
    proxyServices =
        Container.instance(contextId).services().get(ProxyServices.class);
    // hierarchy order
    if (additionalInterfaces.size() > 1) {
      LinkedHashSet<Class<?>> sorted =
          Proxies.sortInterfacesHierarchy(additionalInterfaces);
      additionalInterfaces.clear();
      additionalInterfaces.addAll(sorted);
    }

    this.proxyInstantiator =
        Container.instance(contextId).services().get(ProxyInstantiator.class);
  }

  static String getProxyName(String contextId, Class<?> proxiedBeanType,
                             Set<? extends Type> typeClosure, Bean<?> bean) {
    TypeInfo typeInfo = TypeInfo.of(typeClosure);
    ...
    if (typeInfo.getSuperClass() == Object.class) {
      ...
      if (typeModified) {
        ...
        // interface only bean.
        StringBuilder name =
            new StringBuilder(typeInfo.getSuperClass().getSimpleName() + "$");
        holder = createCompoundProxyName(contextId, bean, typeInfo, name);
      } else {
        holder = new ProxyNameHolder(
            null, typeInfo.getSuperClass().getSimpleName(), bean);
      }
    }
    ...
    if (proxiedBeanType.equals(Object.class)) {
      ...
      if (superInterface == null) {
        ...
        if (reason != null) {
          ...
          BeanLogger.LOG.generatingProxyToDefaultPackage(
              superInterface, DEFAULT_PROXY_PACKAGE, reason);
        }
      }
    } else {
      ...
      if (reason != null && reason.equals(NO_PACKAGE)) {
        proxyPackage = DEFAULT_PROXY_PACKAGE;
        BeanLogger.LOG.generatingProxyToDefaultPackage(
            proxiedBeanType, DEFAULT_PROXY_PACKAGE, reason);
      } else {
  ...
  private static ProxyNameHolder createCompoundProxyName(String contextId,
    ...
    // for producers, try to determine the most specific class and make sure the
    // proxy starts with the same package and class
    ...
    for (Class<?> type : typeInfo.getInterfaces()) {
      ...
      if (declaringClass != null &&
          declaringClasses.add(declaringClass.getSimpleName())) {
    ...
    //  However, it is safe to share a proxy class for built-in beans of the
    //  same type (e.g. Event)
    if (bean != null && !(bean instanceof AbstractBuiltInBean)) {
      final BeanIdentifier id = Container.instance(contextId)
                                    .services()
                                    .get(ContextualStore.class)
                                    .putIfAbsent(bean);
      int idHash = id.hashCode();
      // add a separator so that WeldDefaultProxyServices can determine the
      // correct full class name by first occurrence of "$"
      name.append("$");
  ...
  private static String getEnclosingPrefix(Class<?> clazz) {
    ...
    return encl == null ? ""
                        : getEnclosingPrefix(encl) + encl.getSimpleName() + '$';
  }
   ...
   */
  public static <T> void setBeanInstance(String contextId, T proxy,
                                         BeanInstance beanInstance,
                                         Bean<?> bean) {
    if (proxy instanceof ProxyObject) {
      ProxyObject proxyView = (ProxyObject)proxy;
      proxyView.weld_setHandler(
          new ProxyMethodHandler(contextId, beanInstance, bean));
    }
  }
  ...
  public void addInterfacesFromTypeClosure(Set<? extends Type> typeClosure,
  ...
  public T create(BeanInstance beanInstance) {
    final T proxy = (System.getSecurityManager() == null)
                        ? run()
                        : AccessController.doPrivileged(this);
    ((ProxyObject)proxy)
        .weld_setHandler(new ProxyMethodHandler(contextId, beanInstance, bean));
    return proxy;
  }
  ...
  public T run() {
    try {
      ...
      boolean hasConstrutedField =
          SecurityActions.hasDeclaredField(proxyClass, CONSTRUCTED_FLAG_NAME);
      if (hasConstrutedField != useConstructedFlag()) {
        // The proxy class was created with a different type of
        // ProxyInstantiator
        ProxyInstantiator newInstantiator =
            ProxyInstantiator.Factory.create(!hasConstrutedField);
        BeanLogger.LOG.creatingProxyInstanceUsingDifferentInstantiator(
            proxyClass, newInstantiator, proxyInstantiator);
        return newInstantiator.newInstance(proxyClass);
      }
      return proxyInstantiator.newInstance(proxyClass);
    } catch (InstantiationException e) {
      throw new DefinitionException(
          BeanLogger.LOG.proxyInstantiationFailed(this), e.getCause());
    } catch (IllegalAccessException e) {
      throw new DefinitionException(
          BeanLogger.LOG.proxyInstantiationBeanAccessFailed(this),
          e.getCause());
    }
  }
  ...
  public Class<T> getProxyClass() {
    ...
    Class<?> originalClass =
        bean != null ? bean.getBeanClass() : proxiedBeanType;
    ...
    try {
      ...
      try {
        ...
        try {
          proxyClass =
              cast(proxyServices.loadClass(originalClass, proxyClassName));
        } catch (ClassNotFoundException e2) {
          BeanLogger.LOG.catchingDebug(e1);
          throw BeanLogger.LOG.unableToLoadProxyClass(bean, proxiedBeanType,
                                                      e1);
        }
      }
    }
   ...
   */
  protected String getBaseProxyName() { return baseProxyName; }
   ...
   */
  protected String getProxyNameSuffix() { return PROXY_SUFFIX; }

  private void addDefaultAdditionalInterfaces() {
    additionalInterfaces.add(Serializable.class);
    // add a marker interface denoting that the resulting class uses weld
    // internal contructs
  ...
  private Class<T> createProxyClass(Class<?> originalClass,
                                    String proxyClassName) throws Exception {
    ...
    final int accessFlags = AccessFlag.of(AccessFlag.PUBLIC, AccessFlag.SUPER,
                                          AccessFlag.SYNTHETIC);
    if (getBeanType().isInterface()) {
      proxyClassType =
          newClassFile(proxyClassName, accessFlags, Object.class.getName());
      proxyClassType.addInterface(getBeanType().getName());
    } else {
      proxyClassType =
          newClassFile(proxyClassName, accessFlags, getBeanType().getName());
    }
    ...
    List<DeferredBytecode> initialValueBytecode =
        new ArrayList<DeferredBytecode>();

    // Workaround for IBM JVM - the ACC_STATIC flag should only be required for
    // class file with version number 51.0 or above
    ClassMethod staticConstructor = proxyClassType.addMethod(
        AccessFlag.of(AccessFlag.PUBLIC, AccessFlag.STATIC), "<clinit>", "V");
    ...
    ProtectionDomain domain = AccessController.doPrivileged(
        new GetProtectionDomainAction(proxiedBeanType));

    if (proxiedBeanType.getPackage() == null ||
        proxiedBeanType.getPackage().getName().isEmpty() ||
        proxiedBeanType.equals(Object.class)) {
      domain = ProxyFactory.class.getProtectionDomain();
    } else if (System.getSecurityManager() != null) {
      ProtectionDomainCache cache =
          Container.instance(contextId).services().get(
              ProtectionDomainCache.class);
      domain = cache.getProtectionDomainForProxy(domain);
    }
    Class<T> proxyClass =
        cast(toClass(proxyClassType, originalClass, proxyServices, domain));
    BeanLogger.LOG.createdProxyClass(
        proxyClass, Arrays.toString(proxyClass.getInterfaces()));
    return proxyClass;
  }

  private ClassFile newClassFile(String name, int accessFlags,
  ...
  private void dumpToFile(String fileName, byte[] data) {
    ...
    try {
      Files.write(dumpFile.toPath(), data, StandardOpenOption.CREATE,
  ...
  protected void addConstructors(ClassFile proxyClassType,
    ...
    try {
      if (getBeanType().isInterface()) {
        ConstructorUtils.addDefaultConstructor(
            proxyClassType, initialValueBytecode, !useConstructedFlag());
      } else {
        boolean constructorFound = false;
        for (Constructor<?> constructor : AccessController.doPrivileged(
                 new GetDeclaredConstructorsAction(getBeanType()))) {
          if ((constructor.getModifiers() & Modifier.PRIVATE) == 0) {
            constructorFound = true;
            String[] exceptions =
                new String[constructor.getExceptionTypes().length];
            for (int i = 0; i < exceptions.length; ++i) {
              exceptions[i] = constructor.getExceptionTypes()[i].getName();
            }
            ConstructorUtils.addConstructor(
                BytecodeUtils.VOID_CLASS_DESCRIPTOR,
                DescriptorUtils.parameterDescriptors(
                    constructor.getParameterTypes()),
                exceptions, proxyClassType, initialValueBytecode,
                !useConstructedFlag());
          }
        }
        if (!constructorFound) {
  ...
  protected void addFields(ClassFile proxyClassType,
    ...
    // handling
    proxyClassType.addField(AccessFlag.PRIVATE, METHOD_HANDLER_FIELD_NAME,
                            getMethodHandlerType());
    if (useConstructedFlag()) {
      // field used to indicate that super() has been called
      proxyClassType.addField(AccessFlag.PRIVATE, CONSTRUCTED_FLAG_NAME,
  ...
  protected void addMethodsFromClass(ClassFile proxyClassType,
    ...
    try {
      ...
      // In rare cases, the bean class may be abstract - in this case we have to
      // add methods from all interfaces implemented by any abstract class from
      // the hierarchy
      ...
      while (cls != null) {
        ...
        if (isBeanClassAbstract && Modifier.isAbstract(cls.getModifiers())) {
          for (Class<?> implementedInterface :
               Reflections.getInterfaceClosure(cls)) {
            if (!additionalInterfaces.contains(implementedInterface)) {
              addMethods(implementedInterface, proxyClassType,
                         staticConstructor);
            }
          }
        }
          ...
          if (isMethodAccepted(method, getProxySuperclass())) {
            try {
              MethodInformation methodInfo =
                  new RuntimeMethodInformation(method);
  ...
  private void addMethods(Class<?> cls, ClassFile proxyClassType,
                          ClassMethod staticConstructor) {
    for (Method method :
         AccessController.doPrivileged(new GetDeclaredMethodsAction(cls))) {
      if (isMethodAccepted(method, getProxySuperclass())) {
        try {
  ...
  protected void createSpecialMethodBody(ClassMethod proxyClassType,
  ...
  protected void
  addConstructedGuardToMethodBody(final ClassMethod classMethod) {
    addConstructedGuardToMethodBody(classMethod,
                                    classMethod.getClassFile().getSuperclass());
  }
  ...
  protected void addConstructedGuardToMethodBody(final ClassMethod classMethod,
    ...
    cond.aload(0);
    cond.getfield(classMethod.getClassFile().getName(), CONSTRUCTED_FLAG_NAME,
                  BytecodeUtils.BOOLEAN_CLASS_DESCRIPTOR);
    ...
    cond.invokespecial(className, classMethod.getName(),
                       classMethod.getDescriptor());
    cond.returnInstruction();
    cond.branchEnd(jumpMarker);
  }

  protected void createForwardingMethodBody(ClassMethod classMethod,
                                            MethodInformation method,
                                            ClassMethod staticConstructor) {
   ...
   */
  protected void createInterceptorBody(ClassMethod classMethod,
                                       MethodInformation method,
                                       ClassMethod staticConstructor) {
   ...
   * @param bytecodeMethodResolver The resolver that returns the method to
   *     invoke
   */
  protected void
  invokeMethodHandler(ClassMethod classMethod, MethodInformation method,
    ...
    b.aload(0);
    bytecodeMethodResolver.getDeclaredMethod(
        classMethod, method.getDeclaringClass(), method.getName(),
    ...
    b.invokeinterface(
        MethodHandler.class.getName(), INVOKE_METHOD_NAME, LJAVA_LANG_OBJECT,
        new String[] {LJAVA_LANG_OBJECT, LJAVA_LANG_REFLECT_METHOD,
                      LJAVA_LANG_REFLECT_METHOD, "[" + LJAVA_LANG_OBJECT});
  ...
  protected void addSpecialMethods(ClassFile proxyClassType,
    ...
    try {
      ...
      Method getInstanceMethod =
          TargetInstanceProxy.class.getMethod("weld_getTargetInstance");
      Method getInstanceClassMethod =
          TargetInstanceProxy.class.getMethod("weld_getTargetClass");

      MethodInformation getInstanceMethodInfo =
          new RuntimeMethodInformation(getInstanceMethod);
      createInterceptorBody(proxyClassType.addMethod(getInstanceMethod),
                            getInstanceMethodInfo, staticConstructor);

      MethodInformation getInstanceClassMethodInfo =
          new RuntimeMethodInformation(getInstanceClassMethod);
      createInterceptorBody(proxyClassType.addMethod(getInstanceClassMethod),
                            getInstanceClassMethodInfo, staticConstructor);

      Method setMethodHandlerMethod =
          ProxyObject.class.getMethod("weld_setHandler", MethodHandler.class);
      generateSetMethodHandlerBody(
          proxyClassType.addMethod(setMethodHandlerMethod));

      Method getMethodHandlerMethod =
          ProxyObject.class.getMethod("weld_getHandler");
      generateGetMethodHandlerBody(
          proxyClassType.addMethod(getMethodHandlerMethod));
    } catch (Exception e) {
      throw new WeldException(e);
    }
  }
   ...
   */
  private void
  addConstructorsForBeanWithPrivateConstructors(ClassFile proxyClassType) {
    ClassMethod ctor = proxyClassType.addMethod(
        AccessFlag.PUBLIC, INIT_METHOD_NAME,
        BytecodeUtils.VOID_CLASS_DESCRIPTOR, LJAVA_LANG_BYTE);
    ...
    ctor = proxyClassType.addMethod(AccessFlag.PUBLIC, INIT_METHOD_NAME,
                                    BytecodeUtils.VOID_CLASS_DESCRIPTOR,
                                    LJAVA_LANG_BYTE, LJAVA_LANG_BYTE);
  ...
  protected Class<?> getProxiedBeanType() { return proxiedBeanType; }

  protected void getMethodHandlerField(ClassFile file, CodeAttribute b) {
    b.getfield(file.getName(), METHOD_HANDLER_FIELD_NAME,
               DescriptorUtils.makeDescriptor(getMethodHandlerType()));
  }
  ...
  /**
   * @return {@code true} if {@link ProxyInstantiator} is used to instantiate
   *     proxy instances
   */
  protected boolean isUsingProxyInstantiator() { return true; }

  /**
   * @return {@code true} if {@link #CONSTRUCTED_FLAG_NAME} should be used
   */
  private boolean useConstructedFlag() {
    return !isUsingProxyInstantiator() ||
        proxyInstantiator.isUsingConstructor();
  }

  /**
   * Delegates proxy creation via {@link ProxyServices} to the integrator or to
   * our own implementation.
   */
  protected Class<?> toClass(ClassFile ct, Class<?> originalClass,
    ...
    try {
      ...
      Class<?> result;
      if (domain == null) {
        result = proxyServices.defineClass(originalClass, ct.getName(),
                                           bytecode, 0, bytecode.length);
      } else {
        result = proxyServices.defineClass(
            originalClass, ct.getName(), bytecode, 0, bytecode.length, domain);
      }
      return result;
  ...
  /**
   * When creating a proxy class name we can sometimes determine it's package as
   * well.
   */
  private static class ProxyNameHolder {
    ...
    private ProxyNameHolder(String packageName, String className,
                            Bean<?> bean) {
      this.packageName = packageName;
      if (className == null) {
        throw BeanLogger.LOG.tryingToCreateProxyNameHolderWithoutClassName(
            bean.getBeanClass());
      }

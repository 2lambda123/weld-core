 ...
 * @author Gavin King
 */
@SuppressFBWarnings(value = {"SE_NO_SUITABLE_CONSTRUCTOR", "SE_BAD_FIELD"},
                    justification = "Uses SerializationProxy")
public class InstanceImpl<T> extends AbstractFacade<T, Instance<T>>
    implements WeldInstance<T>, Serializable {
  ...
  private InstanceImpl(InjectionPoint injectionPoint,
    ...
    if (injectionPoint.getQualifiers().isEmpty() &&
        Object.class.equals(getType())) {
      // Do not prefetch the beans for Instance<Object> with no qualifiers
      allBeans = null;
      bean = null;
    } else {
      this.allBeans = resolveBeans();
      // Optimization for the most common path - non-null bean means we are not
      // unsatisfied not ambiguous
      if (allBeans.size() == 1) {
    ...
    this.currentInjectionPoint =
        beanManager.getServices().getRequired(CurrentInjectionPoint.class);
    ...
    this.ip = new DynamicLookupInjectionPoint(getInjectionPoint(), getType(),
                                              getQualifiers());
  ...
  public String toString() {
    return Formats.formatAnnotations(getQualifiers()) + " Instance<" +
        Formats.formatType(getType()) + ">";
  }

  public Iterator<T> iterator() { return new InstanceImplIterator(allBeans()); }

  public boolean isAmbiguous() { return allBeans().size() > 1; }

  public boolean isUnsatisfied() { return allBeans().isEmpty(); }

  public WeldInstance<T> select(Annotation... qualifiers) {
    return selectInstance(this.getType(), qualifiers);
  }

  public <U extends T> WeldInstance<U> select(Class<U> subtype,
                                              Annotation... qualifiers) {
    return selectInstance(subtype, qualifiers);
  }

  public <U extends T> WeldInstance<U> select(TypeLiteral<U> subtype,
                                              Annotation... qualifiers) {
  ...
  public <X> WeldInstance<X> select(Type subtype, Annotation... qualifiers) {
    ...
    // This cast should be safe, we make sure that this method is only invoked
    // on WeldInstance<Object> and any type X will always extend Object
    return (WeldInstance<X>)selectInstance(subtype, qualifiers);
  }

  private <U extends T> WeldInstance<U>
  selectInstance(Type subtype, Annotation[] newQualifiers) {
    InjectionPoint modifiedInjectionPoint = new FacadeInjectionPoint(
        getBeanManager(), getInjectionPoint(), Instance.class, subtype,
        getQualifiers(), newQualifiers);
    return new InstanceImpl<U>(modifiedInjectionPoint, getCreationalContext(),
                               getBeanManager());
  }
  ...
  public void destroy(T instance) {
    ...
    // Attempt to destroy instance which is either a client proxy or a dependent
    // session bean proxy
    if (instance instanceof ProxyObject) {
      ...
      if (proxy.weld_getHandler() instanceof ProxyMethodHandler) {
        ProxyMethodHandler handler =
            (ProxyMethodHandler)proxy.weld_getHandler();
        Bean<?> bean = handler.getBean();
        if (isSessionBeanProxy(instance) &&
    ...
    // Attempt to destroy dependent instance which is neither a client proxy nor
    // a dependent session bean proxy
    destroyDependentInstance(instance);
  }
  ...
  public Iterable<Handler<T>> handlers() {
    return new Iterable<WeldInstance.Handler<T>>() {
      @Override
      public Iterator<org.jboss.weld.inject.WeldInstance.Handler<T>>
      iterator() {
  ...
  public Comparator<Handler<?>> getPriorityComparator() {
    return new PriorityComparator(
        getBeanManager().getServices().get(AnnotationApiAbstraction.class));
  }
  ...
  private T getBeanInstance(Bean<?> bean) {
    final ThreadLocalStackReference<InjectionPoint> stack =
        currentInjectionPoint.pushConditionally(ip,
                                                isRegisterableInjectionPoint());
    try {
      return Reflections.<T>cast(getBeanManager().getReference(
          bean, getType(), getCreationalContext(), false));
    } finally {
      stack.pop();
    }
  }
  ...
  private Set<Bean<?>> resolveBeans() {
    // Perform typesafe resolution, and possibly attempt to resolve the
    // ambiguity
    ...
    return beanResolver.resolve(beanResolver.resolve(
  ...
  private static class SerializationProxy<T>
      extends AbstractFacadeSerializationProxy<T, Instance<T>> {

    private static final long serialVersionUID = 9181171328831559650L;

    public SerializationProxy(InstanceImpl<T> instance) { super(instance); }

    private Object readResolve() throws ObjectStreamException {
      return InstanceImpl.of(getInjectionPoint(), getCreationalContext(),
                             getBeanManager());
    }
  }
  ...
  class InstanceImplIterator extends BeanIterator<T> {
    ...
    private InstanceImplIterator(Set<Bean<?>> beans) { super(beans); }
  ...
  class HandlerIterator extends BeanIterator<Handler<T>> {
    ...
    private HandlerIterator(Set<Bean<?>> beans) { super(beans); }

    @Override
    public Handler<T> next() {
      Bean<?> bean = delegate.next();
      return new HandlerImpl<>(
          () -> getBeanInstance(bean), InstanceImpl.this, bean);
    }
  }
  ...
  private static class HandlerImpl<T> implements Handler<T> {
    ...
    @Override
    public T get() {
      // attempting to resolve the contextual reference after it has been
      // destroyed should throw an ISE
      if (isDestroyed.get()) {
        throw BeanLogger.LOG
            .tryingToResolveContextualReferenceAfterDestroyWasInvoked(this);
      }
      if (!value.isAvailable() && instance.get() == null) {
        // Contextual reference cannot be obtained if the producing Instance
        // does not exist

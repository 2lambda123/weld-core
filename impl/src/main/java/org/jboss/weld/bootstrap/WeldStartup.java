...
public class WeldStartup {
  ...
  public WeldRuntime startContainer(String contextId, Environment environment,
    ...
    WeldConfiguration configuration =
        new WeldConfiguration(registry, deployment);
    registry.add(WeldConfiguration.class, configuration);

    String finalContextId = BeanDeployments.getFinalId(
        contextId, registry.get(WeldConfiguration.class)
                       .getStringProperty(ROLLING_UPGRADES_ID_DELIMITER));
    ...
    String vetoTypeRegex = configuration.getStringProperty(
        ConfigurationKey.VETO_TYPES_WITHOUT_BEAN_DEFINING_ANNOTATION);
    if (!vetoTypeRegex.isEmpty()) {
      this.extensions.add(
          MetadataImpl.from(new WeldVetoExtension(vetoTypeRegex)));
    }
    if (UnusedBeans.isEnabled(configuration)) {
      this.extensions.add(MetadataImpl.from(new WeldUnusedMetadataExtension()));
    }

    // Finish the rest of registry init, setupInitialServices() requires already
    // changed finalContextId
    ...
    if (!proxyServices.supportsClassDefining()) {
      throw BootstrapLogger.LOG.proxyServicesWithoutClassDefining(
          proxyServices.getClass().getName());
    }
    // if we use our own ProxyServices impl, we need to crack open CL for JDK 8
    // impl note that this is no-op call for JDK 11+
    ...
    this.deploymentManager =
        BeanManagerImpl.newRootManager(finalContextId, "deployment", registry);

    Container.initialize(
        finalContextId, deploymentManager,
    ...
    this.deploymentVisitor = new DeploymentVisitor(
        deploymentManager, environment, deployment, contexts, bdaMapping);

    if (deployment instanceof CDI11Deployment) {
      registry.add(
          BeanManagerLookupService.class,
          new BeanManagerLookupService((CDI11Deployment)deployment,
                                       bdaMapping.getBdaToBeanManagerMap()));
    } else {
    ...
    WeldRuntime weldRuntime = new WeldRuntime(
        finalContextId, deploymentManager, bdaMapping.getBdaToBeanManagerMap());
    tracker.end();
    return weldRuntime;
  }
  ...
  private void checkApiVersion() {
    if (Bean.class.getInterfaces().length == 1) {
      // this means Bean only extends Contextual - since CDI 1.1 Bean also
      // extends BeanAttributes CDI 1.0 API is detected on classpath - since
      // that would result in obscure exception later, we throw an appropriate
      // exception right now
  ...
  private void setupInitialServices() {
    ...
    ClassTransformer classTransformer =
        new ClassTransformer(store, cache, reflectionCache, contextId);
  ...
  private void addImplementationServices(ServiceRegistry services) {
    ...
    final WeldConfiguration configuration =
        services.get(WeldConfiguration.class);
    services.add(SlimAnnotatedTypeStore.class,
                 new SlimAnnotatedTypeStoreImpl());
    if (services.get(ClassTransformer.class) == null) {
      throw new IllegalStateException(ClassTransformer.class.getSimpleName() +
                                      " not installed.");
    }
    services.add(MemberTransformer.class,
                 new MemberTransformer(services.get(ClassTransformer.class)));
    services.add(MetaAnnotationStore.class,
                 new MetaAnnotationStore(services.get(ClassTransformer.class)));

    BeanIdentifierIndex beanIdentifierIndex = null;
    if (configuration.getBooleanProperty(
            ConfigurationKey.BEAN_IDENTIFIER_INDEX_OPTIMIZATION)) {
      beanIdentifierIndex = new BeanIdentifierIndex();
      services.add(BeanIdentifierIndex.class, beanIdentifierIndex);
    }

    services.add(ContextualStore.class,
                 new ContextualStoreImpl(contextId, beanIdentifierIndex));
    ...
    services.add(SpecializationAndEnablementRegistry.class,
                 new SpecializationAndEnablementRegistry());
    services.add(MissingDependenciesRegistry.class,
                 new MissingDependenciesRegistry());
    ...
    if (!services.contains(HttpContextActivationFilter.class)) {
      services.add(HttpContextActivationFilter.class,
                   AcceptingHttpContextActivationFilter.INSTANCE);
    }
    services.add(ProtectionDomainCache.class, new ProtectionDomainCache());

    services.add(ProxyInstantiator.class,
                 ProxyInstantiator.Factory.create(configuration));

    services.add(ObserverNotifierFactory.class,
                 DefaultObserverNotifierFactory.INSTANCE);

    services.add(ResourceInjectionFactory.class,
                 new ResourceInjectionFactory());
    ...
    Validator validator;
    if (configuration.getBooleanProperty(
            ConfigurationKey.CONCURRENT_DEPLOYMENT) &&
        services.contains(ExecutorServices.class)) {
      validator = new ConcurrentValidator(
          modules.getPluggableValidators(), executor,
          UnusedBeans.isEnabled(configuration) ? new ConcurrentHashMap<>()
                                               : null);
    } else {
      validator = new Validator(
          modules.getPluggableValidators(),
    ...
    services.add(GlobalObserverNotifierService.class,
                 observerNotificationService);
    ...
    int preloaderThreadPoolSize = configuration.getIntegerProperty(
        ConfigurationKey.PRELOADER_THREAD_POOL_SIZE);
    if (preloaderThreadPoolSize > 0 &&
        Permissions.hasPermission(Permissions.MODIFY_THREAD_GROUP)) {
      preloader = new ContainerLifecycleEventPreloader(
          preloaderThreadPoolSize,
          observerNotificationService.getGlobalLenientObserverNotifier());
    }
    services.add(
        ContainerLifecycleEvents.class,
        new ContainerLifecycleEvents(
            preloader, services.get(RequiredAnnotationDiscovery.class)));
    if (environment.isEEModulesAware()) {
      services.add(BeanDeploymentModules.class,
                   new BeanDeploymentModules(contextId, services));
    }
  }

  // needs to be resolved once extension beans are deployed
  private void
  installFastProcessAnnotatedTypeResolver(ServiceRegistry services) {
    ...
    if (classFileServices != null) {
      ...
      try {
        final FastProcessAnnotatedTypeResolver resolver =
            new FastProcessAnnotatedTypeResolver(
  ...
  public void startInitialization() {
    ...
    ExtensionBeanDeployer extensionBeanDeployer = new ExtensionBeanDeployer(
        deploymentManager, deployment, bdaMapping, contexts);
    ...
    BeforeBeanDiscoveryImpl.fire(deploymentManager, deployment, bdaMapping,
                                 contexts);
    tracker.end();
    ...
    AfterTypeDiscoveryImpl.fire(deploymentManager, deployment, bdaMapping,
                                contexts);
    tracker.end();
  ...
  public void deployBeans() {
    ...
    // we must use separate loops, otherwise cyclic specialization would not
    // work
    for (BeanDeployment deployment : getBeanDeployments()) {
    ...
    AfterBeanDiscoveryImpl.fire(deploymentManager, deployment, bdaMapping,
                                contexts);
    tracker.end();

    // Extensions may have registered beans / observers. We need to flush
    // caches.
    flushCaches();

    // If needed, recreate enablement once again - extensions may have
    // registered interceptors, decorators and alternatives
    if (deployment.getServices()
            .getRequired(GlobalEnablementBuilder.class)
            .isDirty()) {
      for (BeanDeployment beanDeployment : getBeanDeployments()) {
        beanDeployment.createEnablement();
      }
    ...
    for (BeanDeployment beanDeployment : getBeanDeployments()) {
      beanDeployment.getBeanManager()
          .getServices()
          .get(InjectionTargetService.class)
          .initialize();
    ...
    try {
      for (BeanDeployment beanDeployment : getBeanDeployments()) {
        ...
        deployment.getServices()
            .get(Validator.class)
            .validateDeployment(beanManager, beanDeployment);
  ...
  public void endInitialization() {
    ...
    if (UnusedBeans.isEnabled(
            deploymentManager.getServices().get(WeldConfiguration.class))) {
    ...
    if (modules != null) {
      ...
      for (BeanDeploymentModule module : modules) {
        if (!module.isWebModule()) {
          module.fireEvent(Object.class, ContextEvent.APPLICATION_INITIALIZED,
  ...
  createContexts(ServiceRegistry services) {
    ...
    /*
     * Register a full set of bound and unbound contexts. Although we may not
     * use all of these (e.g. if we are running in a servlet environment) they
     * may be useful for an application.
     */
    Set<Annotation> boundQualifires = ImmutableSet.<Annotation>builder()
                                          .addAll(Bindings.DEFAULT_QUALIFIERS)
                                          .add(BoundLiteral.INSTANCE)
                                          .build();
    Set<Annotation> unboundQualifiers = ImmutableSet.<Annotation>builder()
                                            .addAll(Bindings.DEFAULT_QUALIFIERS)
                                            .add(UnboundLiteral.INSTANCE)
                                            .build();
    contexts.add(new ContextHolder<ApplicationContext>(
        new ApplicationContextImpl(contextId), ApplicationContext.class,
        unboundQualifiers));
    contexts.add(new ContextHolder<SingletonContext>(
        new SingletonContextImpl(contextId), SingletonContext.class,
        unboundQualifiers));
    contexts.add(new ContextHolder<BoundSessionContext>(
        new BoundSessionContextImpl(contextId, beanIdentifierIndex),
        BoundSessionContext.class, boundQualifires));
    contexts.add(new ContextHolder<BoundConversationContext>(
        new BoundConversationContextImpl(contextId, services),
        BoundConversationContext.class, boundQualifires));
    contexts.add(new ContextHolder<BoundRequestContext>(
        new BoundRequestContextImpl(contextId), BoundRequestContext.class,
        boundQualifires));
    contexts.add(new ContextHolder<RequestContext>(
        new RequestContextImpl(contextId), RequestContext.class,
        unboundQualifiers));
    contexts.add(new ContextHolder<DependentContext>(
        new DependentContextImpl(services.get(ContextualStore.class)),
        DependentContext.class, unboundQualifiers));

    services.get(WeldModules.class)
        .postContextRegistration(contextId, services, contexts);
  ...
  verifyServices(ServiceRegistry services,
    ...
    for (Class<? extends Service> serviceType : requiredServices) {
      if (!services.contains(serviceType)) {
        throw BootstrapLogger.LOG.unspecifiedRequiredService(
            serviceType.getName(), target);
      }
    }
  }
  ...
  startExtensions(Iterable<Metadata<Extension>> extensions) {
    ...
    // TODO WELD-1624 Weld should fire BeforeBeanDiscovery to allow extensions
    // to register additional scopes
    final Set<Class<? extends Annotation>> beanDefiningAnnotations =
        ImmutableSet.of(
            // built-in scopes
            Dependent.class, RequestScoped.class, ConversationScoped.class,
  ...
  /**
   * Right now, only session and conversation scoped beans (except for built-in
   * beans) are taken into account.
   *
    ...
    for (BeanDeployment beanDeployment : getBeanDeployments()) {
      for (Bean<?> bean : beanDeployment.getBeanManager().getBeans()) {
        if (!(bean instanceof AbstractBuiltInBean<?>) &&
            (bean.getScope().equals(SessionScoped.class) ||
             bean.getScope().equals(ConversationScoped.class))) {
          beans.add(bean);
        }
      }
    }
  ...
  BeanManagerImpl getDeploymentManager() { return deploymentManager; }

  BeanDeploymentArchiveMapping getBdaMapping() { return bdaMapping; }

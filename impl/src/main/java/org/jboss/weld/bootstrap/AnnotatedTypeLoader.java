...
/**
 * Takes care of loading a class, creating {@link BackedAnnotatedType} and
 * creating {@link SlimAnnotatedTypeContext}.
 *
...
class AnnotatedTypeLoader {
  ...
  AnnotatedTypeLoader(BeanManagerImpl manager, ClassTransformer transformer,
    ...
    this.missingDependenciesRegistry =
        manager.getServices().get(MissingDependenciesRegistry.class);
    this.containerLifecycleEvents = containerLifecycleEvents;
  }

  /**
   * Creates a new {@link SlimAnnotatedTypeContext} instance for a class with
   * the specified class name. This method may return null if there is a problem
   * loading the class or this class is not needed for further processing (e.g.
   * an annotation or a vetoed class).
   *
   * @param className the specified class name
   * @param bdaId the identifier of the bean archive this class resides in
   * @return a new {@code SlimAnnotatedTypeContext} for a specified class or
   *     null
   */
  public <T> SlimAnnotatedTypeContext<T> loadAnnotatedType(String className,
                                                           String bdaId) {
    return loadAnnotatedType(this.<T>loadClass(className), bdaId);
  }

  /**
   * Creates a new {@link SlimAnnotatedTypeContext} instance for the given
   * class. This method may return null if there is a problem loading the class
   * or this class is not needed for further processing (e.g. an annotation or a
   * vetoed class).
   *
   ...
   *     null
   */
  public <T> SlimAnnotatedTypeContext<T> loadAnnotatedType(Class<T> clazz,
                                                           String bdaId) {
  ...
  protected <T> SlimAnnotatedType<T> internalLoadAnnotatedType(Class<T> clazz,
                                                               String bdaId) {
    // do not load if clazz is null, an annotation or anonymous/local class
    if (clazz != null && !clazz.isAnnotation() && !clazz.isAnonymousClass() &&
        !clazz.isLocalClass()) {
      try {
        if (!Beans.isVetoed(
                clazz)) { // may throw ArrayStoreException - see bug
                          // http://bugs.sun.com/view_bug.do?bug_id=7183985
          containerLifecycleEvents.preloadProcessAnnotatedType(clazz);
          try {
            return classTransformer.getBackedAnnotatedType(clazz, bdaId);
          } catch (ResourceLoadingException e) {
            missingDependenciesRegistry.handleResourceLoadingException(
                clazz.getName(), e);
          }
        }
  ...
  /**
   * Determines if the class is a MR JAR class, e.g. checks if its path starts

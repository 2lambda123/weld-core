...
public class BeanConfiguratorImpl<T>
    implements WeldBeanConfigurator<T>, Configurator<Bean<T>> {
  ...
  public BeanConfiguratorImpl(Class<?> defaultBeanClass,
    ...
    this.beanManager = beanDeploymentFinder.getOrCreateBeanDeployment(beanClass)
                           .getBeanManager();
  ...
  @Override
  public <U extends T> WeldBeanConfigurator<U>
  ...
  public <U extends T> WeldBeanConfigurator<U> read(AnnotatedType<U> type) {
    ...
    final InjectionTarget<T> injectionTarget =
        cast(beanManager.getInjectionTargetFactory(type).createInjectionTarget(
            null));
  ...
  public Bean<T> complete() {
    if (createCallback == null) {
      // not callback specified, Weld does not know how to instantiate this new
      // custom bean
      throw BeanLogger.LOG.noCallbackSpecifiedForCustomBean(
          "bean [" + beanClass.toString() + ", with types: " +
          Formats.formatTypes(attributes.types) + ", and qualifiers: " +
          Formats.formatAnnotations(attributes.qualifiers) + "]");
    }
  ...
  static final class CreateCallback<T> {
    ...
    static <T> CreateCallback<T>
    fromProduceWith(Function<Instance<Object>, T> callback) {
    ...
    static <T> CreateCallback<T>
    fromCreateWith(Function<CreationalContext<T>, T> callback) {
      return new CreateCallback<T>(null, callback, null);
    }

    CreateCallback(Supplier<T> simple, Function<CreationalContext<T>, T> create,
                   Function<Instance<Object>, T> instance) {
      this.simple = simple;
      this.create = create;
      this.instance = instance;
    }

    private T create(Bean<?> bean, CreationalContext<T> ctx,
    ...
    private Instance<Object> createInstance(Bean<?> bean,
    ...
    CreationalContext<T> getCreationalContext() { return creationalContext; }
  }
  ...
  static class GuardedInstance<T> extends ForwardingWeldInstance<T> {
    ...
    private <TYPE> WeldInstance<TYPE> wrap(Type subtype,
                                           WeldInstance<TYPE> delegate) {
      if (subtype != null && InjectionPoint.class.equals(subtype)) {
        throw BeanLogger.LOG.cannotInjectInjectionPointMetadataIntoNonDependent(
            bean);
      }
  ...
  static final class DestroyCallback<T> {
    ...
    static <T> DestroyCallback<T>
    fromDispose(BiConsumer<T, Instance<Object>> callback) {
      return new DestroyCallback<>(callback, null);
    }

    static <T> DestroyCallback<T>
    fromDestroy(BiConsumer<T, CreationalContext<T>> callback) {
      return new DestroyCallback<>(null, callback);
    }

    public DestroyCallback(BiConsumer<T, Instance<Object>> dispose,
                           BiConsumer<T, CreationalContext<T>> destroy) {
   ...
   */
  static class ImmutableBean<T> extends ForwardingBeanAttributes<T>
      implements WeldBean<T>, PassivationCapable {
    ...
    public void destroy(T instance, CreationalContext<T> creationalContext) {
      ...
      if (creationalContext instanceof CreationalContextImpl) {
        // release dependent instances linked with this bean but avoid double
        // invocation
        ((CreationalContextImpl<T>)creationalContext).release(this, instance);
        // in some cases, the CC for create and destroy callbacks might differ;
        // hence this special handling
        CreationalContext<T> createCallbackCreationalContext =
            createCallback.getCreationalContext();
        if (createCallbackCreationalContext != null &&
            createCallbackCreationalContext != creationalContext) {
    ...
    public String toString() {
      return "Configurator Bean [" + getBeanClass().toString() +
          ", types: " + Formats.formatTypes(getTypes()) +
          ", qualifiers: " + Formats.formatAnnotations(getQualifiers()) + "]";
    }
  }
}

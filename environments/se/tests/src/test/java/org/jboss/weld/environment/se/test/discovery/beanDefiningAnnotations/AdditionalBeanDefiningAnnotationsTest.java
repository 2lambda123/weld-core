...
/**
 * Adds new bean defining annotations in Weld SE, then leaves discovery on and
 * asserts that beans were found.
 *
 * Also tests that you can add new BDA via purely CDI SE container properties
 * and via system properties.
 *
 * @see WELD-2523
 * @see WELD-2639
 *
...
public class AdditionalBeanDefiningAnnotationsTest {
  ...
  public static Archive<?> createTestArchive() {
    return ClassPath.builder()
        .add(ShrinkWrap.create(BeanArchive.class)
                 .addPackage(
                     AdditionalBeanDefiningAnnotationsTest.class.getPackage()))
        .build();
  }
  ...
  public void testDeclarationViaPropertiesWithWrongValue() {
    Weld weld =
        new Weld()
            ...
            .setBeanDiscoveryMode(BeanDiscoveryMode.ANNOTATED)
            .addPackages(Bar.class.getPackage())
            .addProperty(Weld.ADDITIONAL_BEAN_DEFINING_ANNOTATIONS_PROPERTY,
                         "foo");
  ...
  public void testDeclarationViaPropertiesWithCorruptedSet() {
    // we log warning on each wrong item in the list, so this should pass and
    // just skip wrong value
  ...
  private void setupSystemProperty(boolean correctClass, boolean wrongType,
                                   boolean nonExistentClass) {
    ...
    props.setProperty(Weld.ADDITIONAL_BEAN_DEFINING_ANNOTATIONS_PROPERTY,
                      builder.toString());
  }

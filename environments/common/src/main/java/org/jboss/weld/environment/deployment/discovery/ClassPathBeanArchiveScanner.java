...
/**
 * Scans all the class path entries. Implicit bean archives which don't contain
 * a beans.xml file are also supported.
 *
 * @author Martin Kouba
...
public class ClassPathBeanArchiveScanner extends AbstractBeanArchiveScanner {
  ...
  private static final Logger logger =
      Logger.getLogger(ClassPathBeanArchiveScanner.class);

  private static final String BEANS_XML_FOUND_MESSAGE =
      "beans.xml found in {0}";

  private static final String BEANS_XML_NOT_FOUND_MESSAGE =
      "beans.xml not found in {0}";

  private static final String MANIFEST_FILE = "META-INF/MANIFEST.MF";

  private static final Pattern MANIFEST_CLASSPATH_SEPARATOR_PATTERN =
      Pattern.compile(" +");
   ...
   * @param bootstrap
   */
  public ClassPathBeanArchiveScanner(Bootstrap bootstrap) { super(bootstrap); }
  ...
  private void scanDirectory(File entryDirectory,
                             ImmutableList.Builder<ScanResult> results)
      throws IOException {
    // First try to find beans.xml
    File beansXmlFile =
        new File(entryDirectory, AbstractWeldDeployment.BEANS_XML);
  ...
  scanManifestClassPath(URL context, String classPath,
    ...
    for (String entry : entries) {
      ...
      try {
        ...
        if (visitedClassPathEntries.add(entryUrl) &&
            entryUrl.getProtocol().equals("file")) {
